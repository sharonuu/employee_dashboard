...
public class EmployeeRepositoryImpl implements EmployeeRepository {
    ...
    public Employee save(Employee employee) {
        ...
        jdbcTemplate.update(connection -> {
            ...
            ps.setString(6, String.valueOf(employee.getEmployeeStatus()));
            return ps;
        }, keyHolder);
    ...
    public void update(Employee employee) {
        jdbcTemplate.update("UPDATE Employee SET employeeFirstName=?, employeeLastName=?, employeeSalary=?, employeePos=?, employeeStatus=? WHERE employeeId=?",
                employee.getEmployeeFirstName(), employee.getEmployeeLastName(), employee.getEmployeeSalary(), employee.getEmployeePos(), employee.getEmployeeStatus().name(),employee.getEmployeeId());
    }
    ...
    public List<Employee> searchEmployees(Long id, String firstName, String lastName, Float salary, String pos) {
        ...
        return jdbcTemplate.query(sql.toString(), (rs, rowNum) -> {
            ...
            employee1.setEmployeeStatus(EmployeeStatus.valueOf(rs.getString("employeeStatus")));
            return employee1;
        }, params.toArray());
    }
